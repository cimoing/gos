...
生成项目的命令入口基于 Cobra 实现。`schedule` 用于定时任务循环，占位逻辑在 `cmd/api/main.go` 的 `runScheduledJobs` 中扩展。`queue` 用于队列消费循环，占位逻辑在 `processQueueOnce` 中扩展。
...
开启后，启动阶段会初始化 OTLP HTTP trace exporter，HTTP router 会通过 `otelhttp` 生成请求 span。未使用 `--with-otel` 时不会生成相关依赖和代码。
