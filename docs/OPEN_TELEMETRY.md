...
1. metrics 指标采集。
2. logs 日志上报。
3. 数据库 SQL 自动 tracing。
4. Redis、消息队列、外部 HTTP client 自动 tracing。
5. Collector、Jaeger、Tempo 等基础设施部署文件。
```
...
internal/observability
  创建 OTLP HTTP trace exporter、TracerProvider、resource 和 propagator。
...
当前自动产生的 span 主要来自 `otelhttp` 对 HTTP server 的包裹。Usecase、Repository、外部 HTTP client、队列消费等更细粒度 span 需要业务代码按需补充。
...
生产环境一般会把 `debug` exporter 换成 Jaeger、Tempo、OTLP upstream、云厂商 APM 或内部可观测平台。
...
3. 日志中保留 request_id，后续可扩展 trace_id/span_id 注入。
...
```text
1. 将当前 trace_id/span_id 注入 slog 字段。
2. 增加 --with-log-file 生成 lumberjack 文件轮转方案。
3. 提供 otelcol + Loki/Tempo 的 docker compose 示例。
```
...
```text
1. 日志中注入 trace_id/span_id，提升排障关联能力。
2. 为外部 HTTP client 提供 otelhttp.Transport 示例。
3. 为数据库访问增加可选 sql tracing 方案。
4. 提供本地 otelcol + Tempo/Jaeger 示例。
```
